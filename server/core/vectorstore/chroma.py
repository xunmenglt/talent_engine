...
from configs import settings,logger
from server.knowledge import KnowledgeFile
...
class ChromaVectorStore(VectorStoreBase):
    ...
    def search_docs(self, text, top_k, threshold, embedding_field: Literal["query", "question"] = "query", **kwargs)->List[Tuple[Document, float]]:
        """
        按 embedding_field 类型过滤后搜索向量相似度最高的 top_k 条记录。
        :param text: 查询文本
        :param top_k: 返回结果数
        :param threshold: 相似度阈值（目前未应用）
        :param embedding_field: 指定使用 "query" 或 "question" 作为语义字段过滤
        ...
        query_result: QueryResult = self.collection.query(
            ...
            n_results=top_k
            # where={"embedding_field": embedding_field}
        )
